...
class MakeFullResource extends Command
{
    ...
    {
        $modelName = $this->ask('Nombre del modelo (ej. Producto)');
        $tableName = Str::snake(Str::pluralStudly($modelName));
        $fieldsInput = $this->ask('Campos (formato: tipo:nombre, ej. string:nombre, integer:stock, foreignId:categoria_id)');
        $relationsInput = $this->ask('Relaciones (formato: tipo:modelo, ej. belongsTo:Categoria, hasMany:Comentario)', '');
        ...
        // 3. Editar modelo: guarded + relaciones
        ...
        // 4. Crear recurso Filament
        Artisan::call('make:filament-resource', ['name' => $modelName]);
        $this->info(Artisan::output());

        // 5. Modificar Resource: insertar imports y configurar form/table/filters
        $resourcePath = base_path("app/Filament/Resources/{$modelName}Resource.php");
        if (! $fs->exists($resourcePath)) {
            $this->error("No se encontró el archivo de recurso en: $resourcePath");
            return;
        }

        $resourceContent = $fs->get($resourcePath);

        // Eliminar posibles imports viejos antes de <?php
        $resourceContent = preg_replace('/^[\s\S]*?(<\?php)/', '$1', $resourceContent);
        // Insertar use necesarios tras namespace
        ...
        $this->info("✅ Recurso completo para {$modelName} creado.");
    }
}
